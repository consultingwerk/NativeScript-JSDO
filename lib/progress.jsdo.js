...
/*jslint nomen: true*/
// Radu Nicoara, 18.09.2017
// Emulate browser session storage using the nativescript settings API
(function() {
    var emulatedStorage = function() {
        
    }

    emulatedStorage.prototype.getItem = function(key) {
        var appSettings =  require('application-settings');
        return appSettings.getString('smart_jsdo_' + key);
    }

    emulatedStorage.prototype.setItem = function(key, value) {
        var appSettings =  require('application-settings');
        appSettings.setString('smart_jsdo_' + key, value);
    }

    emulatedStorage.prototype.removeItem = function(key) {
        var appSettings =  require('application-settings');
        appSettings.remove('smart_jsdo_' + key);
    }

    global.sessionStorage = new emulatedStorage();
})();
(function (sessionStorage) {
    // Radu Nicoara, 17.08.2017
    // Add Edsel's $ wrapper (remove jQuery dependency)
    $ = function () {
        "use strict";
    };
    $.Deferred = function () {
    ...
    progress.data.Session = function Session(options) {
        ...
        this._openRequest = function (xhr, verb, url, async, callback) {
            var urlPlusCCID,
                that = this;
            // Radu Nicoara, 18.09.2017
            // When running in nativescript, the xhr object will sometimes
            // be undefined in the afterOpenAndAuthorize callback
            // ensure it is a valid reference by falling back to _xhr
            var _xhr = xhr;
            ...
            if (this.clientContextId && (this.clientContextId !== "0")) {
                ...
                if (url.substring(0, this.serviceURI.length) == this.serviceURI) {
                    ...
                    if (index == -1) {
                        ...
                        // Radu Nicoara, 18.09.2017
                        // In NativeScript, we need substring(0, index + 1, otherwise JSESSIONID is appended directly after the URL)
...
(function () {
    ...
    progress.data.AuthenticationProvider.prototype.logout = function () {
        ...
        promise.resolve(this, progress.data.Session.SUCCESS, {});
        return promise;
    };
